//
//  Wundercast3ViewController.swift
//  Study_RxSwift
//
...
typealias Weather = ApiController3.Weather

class WundercastViewController3: UIViewController {
...
//        }
        ...
        let geoSearch = geoLocation.flatMap { location in
            return ApiController3.shared.currentWeather(at: location.coordinate)
        ...
        let retryHandler: (Observable<Error>) -> Observable<Int> = { e in
            return e.enumerated().flatMap { attempt, error -> Observable<Int> in
                ...
                } else if let casted = error as? ApiController3.ApiError, casted == .invalidKey {
...
//                        .map { _ in 1}
        ...
        let textSearch = searchInput.flatMap { text in
            return ApiController3.shared.currentWeather(city: text)
    ...
    func requestKey() {
        ...
        alert.addAction(UIAlertAction(title: "Ok", style: .default) { [weak self] _ in
            ApiController3.shared.apiKey.onNext(self?.keyTextField?.text ?? "")
        })
    ...
    private func showError(error e: Error) {
        guard let e = e as? ApiController3.ApiError else {
